...
- **Never verified against real hardware:** Exact Clone — still the main open risk. Erasing is next after it: the harness runs a real `diskutil eraseDisk` against a throwaway disk image, which is the same command against a device node, but no USB stick has been through it and no erased stick has then been loaded in a CDJ. The update path is now proven end to end against the *published* 1.1 — see [Status](#status) — with one gap left: the **button** has never been pressed. Everything under it has run, but nobody has clicked Install and Relaunch in the GUI and watched the app come back. Disk image save/restore is close behind: heavily covered by the harness against real `hdiutil` images, but never once against a DJ stick or a large library. Also unproven in the running app: the drive registry's picker line and its Application Support file, Compare against two *large* real libraries, and **the picker's disk-image rows, its two file panels, and the image preflight wording** — the engine path below them has 60 checks, the UI above them has none.
...
**The picker scrolls the whole page, and the footer sits outside it.** Every other screen has a fixed amount of chrome and caps its one variable-length list with `.frame(maxHeight:)`. The picker is the exception: its height is set by how many volumes are plugged in, which is not something the window can be resized to suit — `Metrics.windowMinHeight` is 580 and the content wants more. Laid out as a plain `VStack`, the drive rows are what gives. Each row is a stack of single-line labels with no slack, so a stack told to fit them into less space hands each row a smaller slot and the rows draw over one another: at 580×612 with two volumes mounted, names and format badges rendered *outside* their card, on top of the previous row's history line. Fixed in 1.2.

Two things about the fix are load-bearing. The scroll region contains **everything except the footer** — wrapping only the drive lists doesn't work, because as siblings the lists and the fixed chrome still negotiate for height and the lists still lose; as the only flexible child, the scroll region absorbs the whole shortfall. And the footer stays outside it because Continue is the point of the screen and must never scroll out of reach. `.fixedSize` on the rows was tried first and is not needed once the page scrolls, so it isn't there.
...
**Testing the UI: kill the installed copy first.** `/Applications/ThumbPrint.app` and a fresh `Scripts/build.sh` build are both called `ThumbPrint`, so `open`, `osascript … tell process "ThumbPrint"` and `screencapture` will all happily target whichever instance is already running — and the installed one usually is. Three consecutive layout fixes were "verified" against the shipped 1.0 this way on 2026-08-19, producing pixel-identical screenshots that made the fixes look ineffective when in fact they had never been on screen. `ps -eo pid,comm | grep -i thumbprint` before believing anything, and launch the build directly via `<app>/Contents/MacOS/ThumbPrint` rather than through `open`.
...
**Released as 1.1 on 2026-08-19**, and then verified the way a stranger's copy of 1.0 would experience it. The feed fetches anonymously as `v1.1` with one 2,740,827-byte `ThumbPrint-1.1.dmg`; the anonymous download is SHA-256 identical to the built image (`e4c81034…`); the app inside passes `spctl` as `source=Notarized Developer ID`, validates its stapled ticket offline, and satisfies the updater's own pinned requirement — which is the compatibility check that matters, because it is exactly what an installed 1.0 will run before it replaces itself.

Then the whole path, on a scratch copy of the installed 1.0: fetch → "newer than 1.0: true" → download → verify → install, ending with the scratch bundle at version 1.1 and still satisfying the pinned requirement. **The one thing still untested is the button**: that run was driven from a CLI harness, so nobody has clicked Install and Relaunch in the GUI and watched the app quit and come back as 1.1. The relaunch shell script (`scheduleRelaunch`) is the only line of this feature that has never executed.
